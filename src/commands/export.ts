...
/**
 * Generates GraphQL SDL from documented type-like symbols.
 */
function generateGraphQL(moduleSchemas: Map<string, ModuleSchema>): string {
    const lines: string[] = [
        '# Generated by docDocs',
        'schema {',
        '  query: Query',
        '}',
        '',
    ...
    for (const schema of moduleSchemas.values()) {
        for (const [name, symbolSchema] of Object.entries(schema.definitions)) {
            if (symbolSchema.kind !== 'interface' && symbolSchema.kind !== 'type' && symbolSchema.kind !== 'class') {
                continue;
            }
            const desc = symbolSchema.description.replace(/"/g, '\\"');
            lines.push(`"""${desc}"""`);
 ...
 */
/**
 * Exports GraphQL SDL for the workspace.
 */
export async function exportGraphQLCommand(): Promise<void> {
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder) {
        vscode.window.showErrorMessage('No workspace folder open');
        return;
    ...
    const config = await loadConfigForCommand(folder);
    const outputName = config.export?.graphql?.schemaPath ?? 'schema.graphql';

    await vscode.window.withProgress(
        {
            location: vscode.ProgressLocation.Notification,
            title: 'Exporting GraphQL Schema',
            cancellable: true,
        },
        async (progress, token) => {
            const files = await vscode.workspace.findFiles(
            ...
            if (skipCounts.extraction > 0) {
                vscode.window.showWarningMessage(
                    `GraphQL export omitted ${skipCounts.extraction} file(s) (see Problems)`
                );
            }

            const sdl = generateGraphQL(moduleSchemas);
            const outputUri = vscode.Uri.joinPath(folder.uri, '.docdocs', outputName);
            await writeFile(outputUri, sdl);

            vscode.window.showInformationMessage(`GraphQL schema exported to ${outputUri.fsPath}`);
        }
    );
}
...
export function registerExportCommands(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('docdocs.exportGraphQL', exportGraphQLCommand)
    );
}
